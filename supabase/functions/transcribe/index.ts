  ...
  try {
    ...
    if (endpoint && apiKey) {
      try {
        ...
        if (response.ok) { const data = await response.json(); if (data.recordingUrl) audioUrl = data.recordingUrl; }
      } catch { /* fallback */ }
    }
    ...
    if (audioBuffer.byteLength > 25 * 1024 * 1024) throw new Error("File too large (max 25MB)");

    const openaiKey = Deno.env.get("OPENAI_API_KEY");
    if (!openaiKey) throw new Error("OpenAI API key not configured");

    const formData = new FormData();
    formData.append("file", new File([new Blob([audioBuffer])], "recording.mp4"));
      ...
      try {
        const deepseekResponse = await fetch("https://api.deepseek.com/chat/completions", {
          ...
          body: JSON.stringify({
            ...
            messages: [
              { role: "system", content: "You are a helpful assistant that summarizes meeting transcripts. Provide a concise summary." },
              { role: "user", content: `Please provide a concise summary of this meeting transcript: ${transcription}` },
            ],
            stream: false,
          }),
        });
    ...
    const { error: insertError } = await supabase.from("transcripts").insert({ bot_id: botId, content: transcription, summary });
    if (insertError) throw insertError;

    return new Response(JSON.stringify({ success: true, summary }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (error) {
