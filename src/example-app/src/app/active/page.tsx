...
export default function ActiveBotsPage() {
  ...
  const fetchEvents = useCallback(async (botId: string) => {
    const { data } = await supabase.from("bot_events").select("*").eq("bot_id", botId).order("event_time", { ascending: false }).limit(50);
  ...
  const handleDeployNow = async (bot: Bot) => {
    ...
    try {
      ...
      await supabase.from("bots").update({ status: "DEPLOYING", started_at: new Date().toISOString() }).eq("id", bot.id);
    } catch { /* silent */ }
  };
  ...
  if (loading) {
    return <div className="flex h-64 items-center justify-center"><Activity className="size-6 animate-pulse text-muted-foreground" /></div>;
  }
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">Monitor bots currently in meetings</p>
      </div>
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          { label: "Total Active", value: stats.total, icon: BotIcon, dot: null },
          { label: "In Call", value: stats.inCall, icon: null, dot: "bg-green-500" },
          { label: "Waiting", value: stats.waiting, icon: null, dot: "bg-yellow-500" },
          { label: "Errors", value: stats.errors, icon: null, dot: "bg-red-500" },
        ].map((s) => (
          <Card key={s.label} className="py-3">
            <CardContent className="flex items-center gap-3 px-4">
              {s.icon && <s.icon className="size-5 text-muted-foreground" />}
              {s.dot && <div className={cn("size-2.5 rounded-full", s.dot)} />}
              <div>
      ...
      {bots.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-muted-foreground">No active bots. Deploy one from the New Meeting page.</p>
          </CardContent>
        </Card>
      ) : (
        <div className="flex flex-col gap-3">
          {bots.map((bot) => {
            ...
            const status = statusConfig[bot.status] ?? { color: "bg-gray-400", label: bot.status };
            const heartbeatStale = bot.last_heartbeat && Date.now() - new Date(bot.last_heartbeat).getTime() > 30000;
            return (
              <Card key={bot.id} className="py-4">
                <CardContent className="flex items-center justify-between gap-4 px-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium">{bot.meeting_title ?? "Untitled Meeting"}</p>
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className={cn("text-xs", heartbeatStale && "text-red-500")}>{timeAgo(bot.last_heartbeat)}</span>
                        </div>
                        {bot.scheduled_at && bot.status === "READY_TO_DEPLOY" && (
                          <>
                            <span className="text-xs text-muted-foreground">·</span>
                            <span className="text-xs text-muted-foreground">Scheduled: {new Date(bot.scheduled_at).toLocaleString()}</span>
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    {bot.status === "READY_TO_DEPLOY" && bot.scheduled_at && (
                      <Button size="sm" variant="outline" onClick={() => handleDeployNow(bot)}>
                        <Zap className="size-4" /> Deploy Now
                      </Button>
                    )}
                    <Dialog onOpenChange={(open) => { if (open) fetchEvents(bot.id); }}>
                      <DialogTrigger asChild>
                        <Button size="sm" variant="ghost">Details</Button>
                      </DialogTrigger>
                      <DialogContent className="max-w-lg">
                        <DialogHeader><DialogTitle>{bot.meeting_title ?? "Bot Events"}</DialogTitle></DialogHeader>
                            ...
                            <div className="flex flex-col gap-2">
                              {(events[bot.id] ?? []).map((event) => (
                                <div key={event.id} className="flex items-start gap-3 rounded-lg border p-3">
                                  <div className={cn("mt-1 size-2 rounded-full", (statusConfig[event.event_type] ?? {}).color ?? "bg-gray-400")} />
                                  <div className="flex-1">
                                    <p className="text-sm font-medium">{event.event_type}</p>
                                    <p className="text-xs text-muted-foreground">{new Date(event.event_time).toLocaleString()}</p>
                                  </div>
                                </div>
                              ))}
                            </div>
