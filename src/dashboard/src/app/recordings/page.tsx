...
import { Globe, Video, Users, Play, Download, Youtube, FileText, Calendar } from "lucide-react";
...
export default function RecordingsPage() {
      ...
      try {
        ...
        if (endpoint && apiKey) {
          const response = await fetch(
            `${endpoint}/api/bots/${bot.id}/recording`,
            {
              headers: {
  ...
  const handleReupload = async (bot: Bot) => {
    try {
      await fetch(
        `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/youtube-upload`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  if (loading) {
    return (
      <div className="flex h-64 items-center justify-center">
        <Video className="size-6 animate-pulse text-muted-foreground" />
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-4xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          View and download past meeting recordings
        </p>
      </div>
      ...
      {bots.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-muted-foreground">
              No recordings available yet. Completed recordings will appear here.
            </p>
          </CardContent>
        </Card>
      ) : (
              ...
              <TableBody>
                {bots.map((bot) => {
                  const PlatformIcon = bot.platform
                    ...
                    : Globe;
                  const ytStatus = bot.youtube_upload_status
                    ? youtubeStatusConfig[bot.youtube_upload_status] ?? {
                        label: bot.youtube_upload_status,
                  ...
                  return (
                    <TableRow key={bot.id}>
                      ...
                      <TableCell>
                        {ytStatus ? (
                          <Badge variant={ytStatus.variant}>
                            {ytStatus.label}
                          </Badge>
                        ) : (
                      ...
                      <TableCell className="text-right">
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => handleOpenRecording(bot)}
                        >
      ...
      >
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {selectedBot?.meeting_title ?? "Recording"}
            </DialogTitle>
          </DialogHeader>
          {fetchingUrl ? (
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {selectedBot?.youtube_video_id && (
                  <Button asChild size="sm" variant="outline">
                    <a
                      href={`https://www.youtube.com/watch?v=${selectedBot.youtube_video_id}`}
                      target="_blank"
                ...
                {selectedBot &&
                  (!selectedBot.youtube_upload_status ||
                    selectedBot.youtube_upload_status === "failed") && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleReupload(selectedBot)}
                    >
                      <Youtube className="size-4" />
                      Upload to YouTube
                    </Button>
                ...
                <Button asChild size="sm" variant="outline">
                  <a href={`/transcripts?botId=${selectedBot?.id}`}>
